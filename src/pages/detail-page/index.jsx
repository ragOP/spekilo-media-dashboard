...
const DetailPage = () => {
  ...
  const fetchOrders = useCallback(async (page = 1, limit = 10) => {
    try {
      ...
      const response = await fetch(
        `https://skyscale-be.onrender.com/api/${id}/get-order/main-abd?page=${page}&limit=${limit}`
      );
      ...
      if (result.success && result.data && result.data.orders) {
        const transformedData = result.data.orders.map(order => ({
          orderId: order.abdOrderId,
          name: order.fullName,
          email: order.email || 'N/A',
          phone: order.phoneNumber,
          additionalProducts: order.additionalProducts.length > 0 
        ...
        const apiTotalPages = parseInt(result.data.totalPages);
        const apiTotalCount = parseInt(result.data.totalCount || result.data.total || 0);
  ...
  }, [searchQuery, dateFilter, customDateRange.start, customDateRange.end, itemsPerPage, fetchOrders]);
  const recordDetails = {
    id: id || '#REC-045',
  ...
  const exportToCSV = () => {
    const headers = ['Order ID', 'Name', 'Email', 'Phone', 'Additional Products', 'Amount', 'Order Date', 'Place of Birth', 'Gender'];
    const csvContent = [
      headers.join(','),
    ...
    a.href = url;
    a.download = `${recordDetails.id}-orders-page-${currentPage}-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
                  ...
                  <div className="flex items-center justify-center py-12">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <div className="text-center">
                        <p className="font-medium text-red-600 mb-2">Failed to load orders</p>
